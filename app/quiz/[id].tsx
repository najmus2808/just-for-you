...
export default function PlayQuiz() {
  ...
  if (!quiz) {
    return <PlaceholderScreen title="Quiz not found" note="It may have been removed." />;
  }
  ...
  if (questions.length === 0) {
    return (
      <PlaceholderScreen
        ...
        note={`Add a few questions to "${quiz.title}" first.`}
      />
    );
  }
  ...
  if (finished) {
    return (
      <ScreenContainer gradient style={styles.center}>
        ...
        <Button label="Back" variant="ghost" onPress={() => goBack()} style={styles.actionButton} />
      </ScreenContainer>
    );
  }
  ...
  return (
    <ScreenContainer gradient style={styles.container}>
      <Pressable
        ...
        onPress={() => router.push({ pathname: '/quiz/edit', params: { id: quiz.id } })}
        hitSlop={12}
        accessibilityRole="button"
        accessibilityLabel="Edit this quiz"
      >
